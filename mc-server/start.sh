...
get_latest_server() {
  MC_VERSION=$(curl -s "https://api.purpurmc.org/v2/purpur" | jq -r -e .versions[-1])
  LATEST_BUILD=$(curl -s "https://api.purpurmc.org/v2/purpur/$MC_VERSION" | jq -r -e .builds[-1])
  SERVER_JAR_FILENAME=$(curl -s "https://api.purpurmc.org/v2/purpur/$MC_VERSION/$LATEST_BUILD" | jq -r -e .downloads.application.name)
  SERVER_JAR_SHA256=$(curl -s "https://api.purpurmc.org/v2/purpur/$MC_VERSION/$LATEST_BUILD" | jq -r -e .downloads.application.sha256)
  ...
  echo "$SERVER_JAR_SHA256 purpur.jar" > purpursha256.txt
  wget --quiet -O purpur.jar -T 60 $SERVER_JAR_URL
}
...
if [[ -z "$ENABLE_UPDATE" ]]; then
  ...
  if [[ ! -e "purpur.jar" ]]; then
    printf "%s\n" "No server JAR found."
    get_latest_server
  fi

  # We have a purpur.jar, is it valid?
  if [[ $(sha256sum -c purpursha256.txt --status 2>/dev/null) -eq 1 || ! -e "purpursha256.txt" ]]; then
